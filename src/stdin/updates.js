...
module.exports = class UpdatesCommand extends StdinCommand {
	constructor(client) {
		super(client, {
			aliases: ['update'],
			description: 'Forward update information message',
			id: 'updates',
		});
	}
	...
	async run(input) {
		...
		if (!channelId) {
			client.log.warn('Usage: .updates <channelId>');
			return;
		}
		...
		try {
			...
			const messageId = config.shortcuts.updates.messageId;
			...
			client.log.success(`Forwarded updates information to channel ${channelId}`);
