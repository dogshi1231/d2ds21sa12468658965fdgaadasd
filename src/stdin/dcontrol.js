...
module.exports = class DControlCommand extends StdinCommand {
	constructor(client) {
		super(client, {
			aliases: ['directcontrol'],
			description: 'Forward DirectControl guide message',
			id: 'dcontrol',
		});
	}
	...
	async run(input) {
		...
		if (!channelId) {
			client.log.warn('Usage: .dcontrol <channelId>');
			return;
		}
		...
		try {
			...
			const messageId = config.shortcuts.dcontrol.messageId;
			...
			client.log.success(`Forwarded DirectControl guide to channel ${channelId}`);
